...
Liquid 是一门开源的模板语言，由 `Shopify` 创造并用 `Ruby` 实现。`Jekyll` 将 `Liquid`作为自身的模版语言，并且添加了许多对象`object`、标记`tag`和过滤器`filter`。这些新增内容包括代表内容页面的对象、用于在页面中引入内容片段的标记`tag`，以及用于操作字符串和 `URL` 的过滤器。


在`Liquid`中有两种类型的标记：`Output`和 `Tag`
...
&sect;&nbsp;除了对一个已经存在的集合进行循环，你还可以定义一段范围区域内的数字进行循环。这段区域既可以通过文字也可以通过变量数定义得到:
...
> [中文官网：https://liquid.bootcss.com/basics/variations/](https://liquid.bootcss.com/basics/variations/);<br>
